...
export function YearSearch({ aulas, anoNum }: YearSearchProps) {
  ...
  const results = useMemo(() => {
    ...
    return aulas
      .filter((a) => {
        const haystack = normalize(`${a.num} ${a.titulo} ${a.topicos ?? ''}`)
    ...
    <section className="mb-10">
      <label className="block">
        ...
        <input
          ...
          placeholder="ex.: função quadrática, matrizes, vetores…"
          ...
          aria-label={`Buscar lição no ano ${anoNum}`}
        />
      </label>
        ...
        <div className="mt-3 rounded-xl border border-clube-mist-soft/40 bg-clube-surface p-2">
          {results.length === 0 ? (
            <p className="px-3 py-4 text-sm text-clube-mist">
              Nenhuma lição encontrada para "{trimmed}".
            </p>
          ) : (
                ...
                <li key={`${a.trimNum}-${a.num}`}>
                  {a.caminho ? (
                    ...
                    >
                      ...
                      <span className="flex-1 text-clube-ink">{a.titulo}</span>
                    ...
                    <div className="flex items-baseline gap-3 px-3 py-2 text-sm">
                      ...
                      <span className="flex-1 text-clube-mist">{a.titulo}</span>
