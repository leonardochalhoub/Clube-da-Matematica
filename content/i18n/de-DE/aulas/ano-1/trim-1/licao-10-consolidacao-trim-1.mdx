...
Lösen Sie das System <Eq>{`\\begin{cases} \\log_2 x + \\log_2 y = 5 \\\\ x - y = 16 \\end{cases}`}</Eq>.
</Exercicio>
...
Lösen Sie das System: <Eq>{`\\begin{cases} 1 \\leq x < 4 \\\\ (x - 3)^2 \\leq 1 \\end{cases}`}</Eq>
</Exercicio>
...
- **Lektion 41** (Jahr 2): Formaler Grenzwert — wo $\Delta x \to 0$